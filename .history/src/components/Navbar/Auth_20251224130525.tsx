...
const Auth = ({isOpen,type,onClose,ToggleType}:AuthProps) => {
  ...
  isOpen && ( 
    ...
    <input type={field.type} placeholder={field.placeholder}className="text-sm sm:text-base border border-black01/20 px-6 py-3.5 rounded-lg mb-3  lg:mb-5 w-full" />
                )}
              </div>
  )
})}
            ...
            <div className="text-center mt-2 md:mt-5 text-black01">{type === "login" ? "Don’t have an account?" : "Already have an account?"}{" "}
              <span className="text-orange cursor-pointer font-semibold border-b-2 border-transparent hover:border-orange transition-colors duration-300"
                onClick={ToggleType}>{type === "login" ? "Sign Up" : "Log In"}</span>
            </div>
          </form>
    </div>
