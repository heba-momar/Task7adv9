...
interface PopularPackagesCardProps {
    image: string;
    title: string;
    price: string;
    features: Feature[];
}
const PopularPackagesCard = ({ image, title, price, features }:PopularPackagesCardProps) => {
  return (
 <div className="bg-white rounded-3xl shadow-[0px_0px_20px_0px_#0000001A] w-full lg:w-[350px] overflow-hidden transition-all duration-300 hover:shadow-[0px_10px_30px_0px_#00000030] hover:scale-[1.02]">
 <Image src={image} alt={title} width={400} height={300} className="w-full h-[340px] object-center lg:object-cover" />
  ...
  {features.map((feature,index)=>{
   return(
    ...
    <button className="py-3 px-6 border text-orange font-semibold text-[20px] border-orange rounded-[50px] transition-colors duration-300 hover:bg-orange hover:text-white"> Book Now </button>
            </div>
    </div>
  )
}
