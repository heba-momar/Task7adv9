...
const DestinantionsCard = ({image,person,des,title,Available}:DestinantionsCardProps) => {
  return (
        <div className='flex flex-col gap-4 w-full lg:w-[330px] '>
            <div className="overflow-hidden rounded-3xl">
                <img src={image} alt={title}className="h-[404px] w-full object-cover rounded-3xl transform transition-transform duration-500 hover:scale-105" />
            </div>
            <div className='flex flex-col gap-3'>
                ...
                <p className="text-lg leading-[26px]">{des}</p>
            </div>
        </div>
  )
}
