...
interface SpecialOffersPros{
 className?:string ,
 label:string  
}

const SpecialOffers = ({className,label}:SpecialOffersPros) => {
  return (
  ...
  <div className="bg-white/30 lg:w-[569px] flex flex-col text-center p-[30px] rounded-3xl gap-8 z-10">
   ...
   <button  className={`bg-orange font-semibold text-white cursor-pointer px-14 w-fit mx-auto py-2.5  text-[20px]  rounded-full hover:shadow-xl transition ${className}`}>{label}</button>
                </div>
