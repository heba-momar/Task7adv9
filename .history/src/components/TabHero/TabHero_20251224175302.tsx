...
import { filterFields } from "@/data/TabsHero";
...
const [startDate, setStartDate] = useState<Date | null>(null);    
  return (
     <div className="mx-auto p-3 rounded-xl bg-white/20 mt-5 sm:mt-12">
            {/* Tabs */}
   ...
   { key: "public", label: "Public Tours", Icon: PublicTours },
    { key: "private", label: "Private Tours", Icon: PrivateTours },
     ...
     return(
      <button  key={key}  onClick={() => setActiveTab(key as "public" | "private")}
      className={`flex items-center p-2 sm:gap-2 sm:p-4 ${key === "public" ? "rounded-tl-xl" : "rounded-tr-xl"
         ...
         <DatePicker selected={startDate} onChange={(date) => setStartDate(date)} placeholderText="Choose Date"
          dateFormat="DD/MM/YYYY" className="w-[150px] pr-6 border-none outline-none bg-transparent text-black01 text-sm sm:text-lg" calendarClassName="!z-[9999]" />
        <span className="absolute right-1 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400 text-sm sm:text-base w-3">
             <img src="/images/hero/tab/arrow-down.svg"  alt="Search Icon"/>
               </span>
             </div>
                            )}
                        </div>
  ...
  <button className="rounded-lg w-full xl:w-[80px] bg-orange p-2 lg:p-5 xl:ml-3   flex items-center justify-center cursor-pointer">
     <img src="/images/hero/tab/search.png"alt="Search Icon"className="w-4 lg:w-fit"  />
        </button>
            </div>
        </div>
  )
